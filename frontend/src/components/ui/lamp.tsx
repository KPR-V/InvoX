...
const LampDemo: React.FC<LampDemoProps> = ({ Type, Words }) => {
  return (
    <LampContainer>
      <motion.h1
      ...
      <motion.h1
        ...
        className="mt-8 border-b-[1px] border-orange-400 bg-gradient-to-br from-slate-300 to-slate-300 py-4 bg-clip-text text-center font-montserrat text-xl font-medium tracking-tight text-transparent md:text-xl"
      >
        {Words}
      </motion.h1>
    </LampContainer>
...
export const LampContainer = ({
  ...
  return (
    <div
      className={cn(
        "relative  min-h-screen flex flex-col items-center justify-center overflow-hidden bg-zinc-950 w-full rounded-md z-0",
        className
      )}
    >
      <div className="relative flex  w-full  flex-1 scale-y-125 items-center justify-center isolate z-0 ">
        ...
        <div className="absolute inset-auto z-50 h-36 w-[28rem] -translate-y-1/2 rounded-full bg-zinc-950 opacity-50 blur-3xl"></div>
        <motion.div
          initial={{ width: "8rem" }}
          ...
          className="absolute inset-auto z-30 h-36 w-64 -translate-y-[6rem] rounded-full bg-orange-300 blur-2xl"
        ></motion.div>
        <motion.div
          initial={{ width: "15rem" }}
          ...
          className="absolute inset-auto z-50 h-0.5 w-[30rem] -translate-y-[7rem] bg-orange-700 "
        ></motion.div>
